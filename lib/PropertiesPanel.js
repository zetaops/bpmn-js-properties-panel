...
PropertiesPanel.prototype.applyValidationErrors = function(validationErrors, entryNode) {
  ...
  forEach(controlNodes, function(controlNode) {
    ...
    var errorNode = domQuery('[data-invalid]', controlNode.parentNode);

    if (error) {
      valid = false;

      if (!errorNode) {
        var errorHtml = domify('<div data-invalid></div>');
        errorHtml.innerHTML = error;
        controlNode.parentNode.insertBefore(errorHtml, controlNode.nextSibling);

        domClasses(controlNode).toggle('invalid');
      }
    } else {
      if (errorNode) {
        controlNode.parentNode.removeChild(errorNode);
