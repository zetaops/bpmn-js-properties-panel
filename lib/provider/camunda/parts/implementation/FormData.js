    ...
    set: function(element, values, update, bo, bpmnFactory) {
      ...
      if (existingFormField) {
        ...
        if (validationErrors.formFieldId) {
          ...
          existingFormField.id = values.formFieldId;
        }

        existingFormField.type = values.formFieldType || '';

        // set label only if input field is not empty
        if (values.formFieldLabel) {
          existingFormField.label = values.formFieldLabel;
        } else {
          existingFormField.label = undefined;
        }

        // set label only if input field is not empty
        if (values.formFieldDefaultValue) {
          existingFormField.defaultValue = values.formFieldDefaultValue;
        } else {
          existingFormField.defaultValue = undefined;
        }

        update = assign(update, cmdHelper.setList(element, formData, 'fields', formData.formFields));
        ...
        // add new form field to existing form field list
        formData.fields.push(newformField);

        update = assign(update, cmdHelper.setList(element, formData, 'fields', formData.fields));
