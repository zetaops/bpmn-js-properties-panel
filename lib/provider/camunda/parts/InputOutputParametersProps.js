...
module.exports = function(group, element, bpmnFactory) {
  ...
  var removeElement = function(getter, prop, otherProp) {
    return function(element, extensionElements, value, idx) {
      var bo = getBusinessObject(element);
      var inputOutput = (getElements(bo, 'camunda:InputOutput') || [])[0];
      var parameter = (getter(bo) || [])[idx];

      var commands = [];
      commands.push(cmdHelper.removeElementsFromList(element, inputOutput, prop, null, [ parameter ]));

      if (!(inputOutput.get(prop).length-1 && !(inputOutput.get(otherProp) || []).length)) {
        commands.push(extensionElementsHelper.removeEntry(bo, element, inputOutput));
      }
  ...
  var inputEntry = extensionElementsEntry(element, bpmnFactory, {
    ...
    getExtensionElements: function(element) {
      return getInputParameters(getBusinessObject(element));
    },
  ...
  var outputEntry = extensionElementsEntry(element, bpmnFactory, {
    ...
    getExtensionElements: function(element) {
      return getOutputParameters(getBusinessObject(element));
    },
  ...
  group.entries.push(entryFactory.textField({
    ...
    get: function(element, node) {
      ...
      return {
        name: typeof name !== 'undefined' ? name : (getSelected(element, node) || {}).name
      };
    },
